...
export function TaskOverview({ view }: { view: 'jobs' | 'experiments' }) {
  ...
  const title = view === 'jobs' ? '任务概览' : '实验记录'
  const description = view === 'jobs' ? '仅轮询持久化任务状态；关闭浏览器不会停止已提交任务。仅合成验收样例可从“新建回测”提交，正式回测仍受门禁保护。' : '仅显示已原子发布的可复现实验记录；失败与取消任务不被伪装为实验结果。'
  return <div className="task-overview"><section className="page-heading"><div><p className="eyebrow">{view === 'jobs' ? '回测中心 · 后台任务' : '研究实验室 · 可复现记录'}</p><h1>{title}</h1><p>{description}</p></div><span className="static-boundary">只读轮询 · 5 秒</span></section>
