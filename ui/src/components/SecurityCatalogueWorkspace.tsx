...
export function SecurityCatalogueWorkspace() {
  ...
  return <div className="universe-workspace">
    <section className="page-heading"><div><p className="eyebrow">市场与数据 · 自动证券档案</p><h1>资产与合约</h1><p>证券资料由系统从现有美股主表自动建立，你只需搜索、查看，不必填写身份编号、时区或交易规则。</p></div><span className="static-boundary">自动建档 · 只读</span></section>
...
['上市市场', display(selected.exchange)], ['证券类型', selected.asset_type.toLowerCase() === 'etf' ? 'ETF' : selected.asset_type.toLowerCase() === 'stock' ? '股票' : selected.asset_type],
      ...
      ].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><p className="security-record-note">资产池中的正式证券映射与合格研究快照仍需系统核验。不会要求你补填技术字段，也不会把当前主表名单冒充历史指数成分股。</p></> : <div className="empty-state"><Search size={24}/><strong>没有可展示的档案</strong><p>输入代码或名称搜索。</p></div>}</section></div>
    </>}
