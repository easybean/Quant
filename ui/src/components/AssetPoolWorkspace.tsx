...
export function AssetPoolWorkspace({ onNavigate }: { onNavigate?: (page: string) => void }) {
  ...
  if (!pools || !definitions) return <section className="empty-state wide"><LoaderCircle size={24} className="animate-spin"/><strong>正在读取研究资产池</strong><p>仅加载旧版固定标的引用和资产池版本；自动证券目录与研究资格分开。</p></section>
