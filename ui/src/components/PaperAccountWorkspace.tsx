...
export function PaperAccountWorkspace() {
  ...
  return <div className="paper-workspace"><section className="page-heading"><div><p className="eyebrow">组合与账户 · 非执行基础</p><h1>模拟账户与订单台账</h1><p>创建本地模拟账户并只读查看不可变订单、成交、费用和审计事件。当前未连接行情或券商，无法自行产生订单或成交。</p></div><span className="static-boundary">本地台账 · 不可执行</span></section>
    {error && <div className="strategy-error"><AlertCircle size={17}/>{error}</div>}
    <section className="strategy-boundary"><Landmark size={18}/><div><strong>不含撮合、行情或伪 PnL</strong><p>账户初始资金仅是明确配置。台账只能由未来经过校验的流程追加；本页没有“下单”“生成成交”或外部账户同步入口。</p></div></section>
    ...
    {active && <section className="paper-ledger"><header><div><BookOpen size={17}/><strong>{active.name} 的只读台账</strong></div><span>{ledger?.reconciliation.event_count ?? '…'} 条事件</span></header>{ledger ? <><div className="paper-balance"><span>初始现金 <strong>{ledger.reconciliation.initial_cash} {active.base_currency}</strong></span><span>已记录现金变动 <strong>{ledger.reconciliation.recorded_cash_delta}</strong></span><span>账本现金 <strong>{ledger.reconciliation.reconciled_cash} {active.base_currency}</strong></span></div>{ledger.items.length ? ledger.items.map(item => <div className="paper-event" key={item.id}><strong>{item.event_type}</strong><span>{item.dedupe_key}</span><span>现金 {item.cash_delta} · 持仓 {item.position_delta}</span><time>{new Date(item.created_at).toLocaleString()}</time></div>) : <div className="paper-empty"><BookOpen size={22}/><strong>台账尚无订单、成交、费用或审计事件</strong><p>本页不生成示例成交。没有行情、券商或撮合器连接，因此不会自行产生订单。</p></div>}<p className="paper-scope">{ledger.reconciliation.scope}</p></> : <div className="paper-empty"><LoaderCircle size={20} className="animate-spin"/><p>正在读取不可变台账…</p></div>}</section>}
  </div>
}
